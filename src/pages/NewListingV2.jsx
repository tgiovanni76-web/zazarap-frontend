...
export default function NewListingV2() {
  const { t } = useLanguage();
  ...
  const createListingMutation = useMutation({
    ...
    onError: () => {
      toast.error(t('listing.createError'));
      setIsUploading(false);
    }
  });
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.title || !formData.price || !formData.category) {
      toast.error(t('listing.fillRequired'));
      return;
    }
    ...
    try {
      ...
      if (!validationResult.data.allowed) {
        toast.error(t('error.illegal'), { duration: 5000 });
        return;
      }
    } catch (err) {
  ...
  return (
    <div className="py-8 max-w-2xl mx-auto">
      <h2 className="text-3xl font-bold mb-6">{t('listing.createNew')}</h2>
      <form onSubmit={handleSubmit}>
        <label className="zaza-form-label">{t('form.title')}</label>
        <input
          name="title"
        ...
        <label className="zaza-form-label">{t('form.description')}</label>
        <textarea
          name="description"
        ...
        <label className="zaza-form-label">{t('form.price')} (€)</label>
        <input
          name="price"
          type="number"
        ...
        <label className="zaza-form-label">{t('listing.offerPrice')}</label>
        <input
          name="offerPrice"
          type="number"
        ...
        <label className="zaza-form-label">{t('listing.expiryDate')}</label>
        <input
          name="expiresAt"
          type="datetime-local"
        ...
        <label className="zaza-form-label">{t('form.category')}</label>
        <select
        ...
        <label className="zaza-form-label">{t('form.city')}</label>
        <input
          name="city"
          value={formData.city}
